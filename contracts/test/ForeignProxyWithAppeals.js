...
beforeEach("initialize the contract", async function () {
  ...
  const ForeignProxy = await ethers.getContractFactory("RealitioForeignArbitrationProxyWithAppeals", governor);
  ...
  await homeProxy.setForeignProxy(foreignProxy.address, homeChainID);
});
...
it("Should correctly set the home proxy and home chain ID", async () => {
  await expect(foreignProxy.connect(other).setHomeProxy(homeProxy.address, 4)).to.be.revertedWith(
  ...
  expect((await foreignProxy.homeChainId()).toNumber()).to.equal(0, "Incorrect home chain ID");
  // Check that can't set it 2 times.
  await expect(foreignProxy.setHomeProxy(homeProxy.address, 5)).to.be.revertedWith("Home proxy already set");
});
...
it("Should set correct values when requesting arbitration and fire the event", async () => {
  ...
  ).to.be.revertedWith("Not initialized yet");
  await foreignProxy.setHomeProxy(homeProxy.address, homeChainID);
...
it("Should correctly withdraw appeal fees if a dispute had winner/loser", async () => {
  ...
  expect(newBalance1).to.equal(oldBalance1, "The balance of the crowdfunder1 should stay the same (withdraw 0 round)");
  ...
  let contributionInfo = await foreignProxy.getContributionsToSuccessfulFundings(arbitrationID, 0, crowdfunder1Address);
  ...
  expect(newBalance1).to.equal(oldBalance1, "The balance of the crowdfunder1 should stay the same (withdraw 1 round)");
  expect(newBalance2).to.equal(
...
it("Should correctly withdraw appeal fees for multiple answers", async () => {
  ...
  oldBalance = await requester.getBalance();
  await foreignProxy.withdrawFeesAndRewardsForMultipleRulings(arbitrationID, await requester.getAddress(), 0, [41, 45]);
  newBalance = await requester.getBalance();
  expect(newBalance).to.equal(
    oldBalance.add(39),
...
it("Should make governance changes", async () => {
  ...
  await expect(foreignProxy.connect(other).changeTermsOfService("New terms")).to.be.revertedWith(
    "Only governor allowed"
  );
  await foreignProxy.connect(governor).changeTermsOfService("New terms");
  expect(await foreignProxy.termsOfService()).to.equal("New terms", "Incorrect terms of service string");

  await expect(foreignProxy.connect(other).changeMetaEvidence("New meta")).to.be.revertedWith("Only governor allowed");
  await expect(foreignProxy.connect(governor).changeMetaEvidence("New meta"))
    .to.emit(foreignProxy, "MetaEvidence")
    .withArgs(1, "New meta");
  expect(await foreignProxy.metaEvidenceID()).to.equal(1, "Incorrect metaevidence ID");

  await expect(foreignProxy.connect(other).setHomeProxy(homeProxy.address, homeChainID)).to.be.revertedWith(
    "Only governor allowed"
  );
  await foreignProxy.connect(governor).setHomeProxy(homeProxy.address, homeChainID);
  expect(await foreignProxy.homeProxy()).to.equal(homeProxy.address, "Incorrect home proxy address");
